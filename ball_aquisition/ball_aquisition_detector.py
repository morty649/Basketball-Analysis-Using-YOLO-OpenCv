...
class BallAquisitionDetector:
    ...
    def find_minimum_distance_to_points(self,ball_center,player_bbox):
        key_points = self.get_key_basketball_player_assignment_points(player_bbox,ball_center)
        return min(measure_distance(ball_center,key_point) for key_point in key_points) #it gives a list and min of list is taken
    ...
    def calculate_ball_containment_ratio(self,player_bbox,ball_bbox):
        ...
        intersection_y2 = min(py2,by2) # all of this to find the intersection bbox

        if intersection_x2 < intersection_x1 or intersection_y2 < intersection_y1:
            return 0  #if no intersection why we need to track 

        intersection_area = (intersection_x2-intersection_x1)*(intersection_y2-intersection_y1)

        containment_ratio = intersection_area/ball_area #how much % of ball is in player bbox
        return containment_ratio

    def find_best_candidate_for_possession(self,ball_center,player_tracks_frame,ball_bbox): # here it is player tracks for frame
        ...
        for player_id,player_info in player_tracks_frame.items():
            ...
            min_distance = self.find_minimum_distance_to_points(ball_center,player_bbox)
        ...
        # First priority high contaminant players
        if high_containment_players:
            best_candidate = max(high_containment_players,key = lambda x: x[1] ) 
            return best_candidate[0] #Means based #Means based on containment or we can say who has more ball area take player id on containment take player id 
    
        # Second priority regular players
        ...
        if regular_distance_players:
            ...
            if best_candidate[1] < self.possession_threshold:
                return best_candidate[0]  #returns player_id 
